...
// Distinguishes two different "can't proceed" states, matching real HTTP
// semantics instead of collapsing everything into 401:
//   - "unauthenticated": no session at all, or the token itself is invalid/expired
//   - "forbidden": the token is cryptographically valid, but the admin account
//     it points to no longer exists (e.g. deleted directly in D1) - the
//     session was once legitimate but access has since been revoked
export async function getAdminStatus(
  c: Context<AppEnv>
): Promise<{ status: 'ok'; admin: { id: number; username: string } } | { status: 'unauthenticated' } | { status: 'forbidden' }> {
  const token = getCookie(c, TOKEN_COOKIE);
  if (!token) return { status: 'unauthenticated' };

  let payload: { sub?: string; username?: string };
  try {
    payload = (await jwtVerify(token, getSecretKey(c.env))).payload as any;
  } catch {
    return { status: 'unauthenticated' };
  }

  const userId = Number(payload.sub);
  const stillExists = await c.env.DB.prepare('SELECT 1 FROM admin_users WHERE id = ?').bind(userId).first();
  if (!stillExists) return { status: 'forbidden' };

  return { status: 'ok', admin: { id: userId, username: String(payload.username) } };
}
