...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ username, onLoggedOut }) => {
  ...
  const [pendingReviewDelete, setPendingReviewDelete] = useState<string | null>(null);
  ...
  const handleCreate = async (payload: any) => {
    // BUG FIX: this used to silently fall back to storing the edit in
    // localStorage (invisible to every real customer and to the merchant on
    // any other device) and still show "created successfully" even when the
    // real database write failed. Now a failed save surfaces a real error
    // and nothing pretends to have worked.
    await createProduct(payload);
    setNotice(`"${payload.name?.en || 'Piece'}" was added to the storefront.`);
    setView({ mode: 'list' });
    await loadData();
  };
  ...
  const handleUpdate = async (id: string, payload: any) => {
    ...
    setNotice(`"${payload.name?.en || 'Piece'}" was updated.`);
    setView({ mode: 'list' });
    await loadData();
  };

  const requestDelete = (product: ProductItem) => {
    setErrorMsg('');
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to delete the piece.');
      setPendingDelete(null);
    } finally {
      setDeleting(false);
    }
  ...
  const handleDeleteReview = (reviewId: string) => {
    setErrorMsg('');
    setPendingReviewDelete(reviewId);
  };

  const confirmDeleteReview = async () => {
    if (!pendingReviewDelete) return;
    try {
      await deleteReviewApi(pendingReviewDelete);
      setNotice('Review removed.');
      await loadData();
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to delete the review.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#0D1929] text-[#E2E6E8] font-body">
      ...
      <main className="max-w-6xl mx-auto px-5 sm:px-8 py-10 space-y-6">
        ...
        {errorMsg && (
          <div className="p-3 border border-red-500/60 bg-red-950/40 text-red-300 text-xs flex items-center justify-between font-mono">
            <span>✕ {errorMsg}</span>
            <button onClick={() => setErrorMsg('')} className="text-red-300/70 hover:text-red-300 cursor-pointer">
              ✕
            </button>
          </div>
        )}
      ...
      <ConfirmDialog
        isOpen={!!pendingReviewDelete}
        title="Delete this review?"
        message="This review will be permanently removed. This can't be undone."
        confirmLabel="Delete"
        cancelLabel="Cancel"
        danger
