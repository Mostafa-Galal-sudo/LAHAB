...
const AssetLibrary: React.FC<AssetLibraryProps> = ({ onClose, onSelect }) => {
  ...
  const upload = async (file?: File) => {
    ...
    try {
      ...
      setNotice(asset.metadata.source === 'github'
        ? 'Committed securely to GitHub. The asset is available now and will be bundled by the next deployment.'
        : 'Upload complete.');
      setProgress(null);
    } catch (reason) {
      setProgress(null);
  ...
  return <div className="fixed inset-0 z-[80] bg-black/75 grid place-items-center p-6" role="dialog" aria-modal="true" aria-label="Asset library">
    <div className="w-full max-w-5xl max-h-[86vh] overflow-hidden bg-[#0D1929] border border-[#D8A065]/40 shadow-2xl flex flex-col">
      <header className="p-5 border-b border-[#E2E6E8]/15 flex justify-between items-center"><div><div className="text-[10px] font-mono text-[#D8A065]">GITHUB-BACKED ASSET ARCHIVE</div><h2 className="font-heading text-xl">ASSET LIBRARY</h2></div><button onClick={onClose} aria-label="Close asset library"><X /></button></header>
      ...
      {notice && <div className="mx-4 mt-3 border border-emerald-500/35 bg-emerald-950/25 p-3 text-xs text-emerald-200" role="status">{notice}</div>}
      <div className="p-4 overflow-y-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {loading ? <p className="text-sm text-[#E2E6E8]/50">Loading assets…</p> : assets.map((asset) => <article key={asset.id} className="border border-[#E2E6E8]/15 bg-[#132238]/50 overflow-hidden group">
          <button onClick={() => onSelect(asset)} className="w-full text-left">
            ...
            <div className="p-3"><div className="text-[11px] font-heading truncate">{asset.fileName}</div><div className="text-[9px] font-mono text-[#E2E6E8]/45 mt-1">{asset.kind} • {(asset.byteSize / 1024).toFixed(1)} KB</div><div className="text-[8px] font-mono uppercase text-emerald-300/70 mt-1">{String(asset.metadata.source ?? 'bundled')}</div><code className="text-[8px] text-[#D8A065]/60 break-all">{asset.id}</code></div>
          </button>
