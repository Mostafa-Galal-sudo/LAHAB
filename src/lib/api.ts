...
async function request<T>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    if (!res.ok) {
      throw new ApiError(`Server returned status ${res.status}: ${text || 'Internal Error'}`, res.status);
    }
    throw new ApiError('Invalid server response format.', res.status);
  }
  if (!res.ok || data.success === false) {
    throw new ApiError(data.error || `Request to ${url} failed`, res.status);
  }
  return data;
}
